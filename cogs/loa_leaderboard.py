...
def record_run(user_id: int, time_seconds: int) -> None:

  # Create file if it doesn't exist
  if not SPEEDRUNS_FILE.exists():
    SPEEDRUNS_FILE.write_text("[]")

  # Load existing runs
  with SPEEDRUNS_FILE.open("r") as f:
    runs = json.load(f)
...
def load_and_sort_runs() -> list[dict]:
  try:
...
class LoaLeaderboard(commands.Cog):
  def __init__(self, bot: commands.Bot):
    self.bot = bot


  @app_commands.command(name="leaderboard", description='View the top 10 runs for the game "Getting Over It"!')
  async def leaderboard(
    ...
    for i in range(min(10, len(runs))):
      ...
      user = await self.bot.fetch_user(run["user_id"])
      formatted_time = format_time(run["time"])
      response += f"{formatted_time} - {user.display_name}\n"
    
    await interaction.response.send_message(response)
  
  @app_commands.command(name="submit-run", description='Submit a run for the game "Getting Over It"!')
...
async def setup(bot: commands.Bot):
  await bot.add_cog(LoaLeaderboard(bot))
